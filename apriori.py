...
while(True):
    print(" Available Dataset Options : 1) dataset_walmart,  2) dataset_7_11,  3) dataset_esty,  4) dataset_k_mart, 0) exit code") 
    option = int(input("select a dataset to run apriori on:"))  
    if option>0:
        data = datasets.get(option)
    else:
        exit(0)
    ...
    product_list = list(data.columns.values)
    C.append(product_list)
    L.append([product_list,list(data.sum(axis=0, skipna=True)/0.2)])
    ...
    while True:
        ...
        for s in temp_c:
            #data[list(s)] =  Association Combination
            ...
            obj = data[list(s)].eq(1, axis='index').all(1)
        ...
        for y in association_values_:
            ...
            if y != x:
                ...
                #check support logic for explanation
                    obj = data[list(z)].eq(1, axis='index').all(1)
